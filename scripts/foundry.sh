...
# Ensure the Python script is downloaded
install_abliterate() {
    ...
    echo "Downloading abliterate.py..."
    ...
    # Fallback URL (change this to your actual raw URL)
    curl -sL "https://huggingface.co/AtomicChat/calib-corpora/resolve/main/tools/abliterate.py" -o "$target"
    if [ -f "$target" ]; then
        echo "Downloaded via fallback URL"
        chmod +x "$target"
        return 0
    fi
...
# Fetch or generate refusal/compliant examples
get_abliteration_examples() {
    ...
    if fetch_one AtomicChat/calib-corpora dataset "abliteration/refusal.txt" "$refusal" 2>/dev/null; then
        ...
        echo "Using built-in refusal examples"
        cat > "$refusal" << 'REFEOF'
...
REFEOF
    ...
    if fetch_one AtomicChat/calib-corpora dataset "abliteration/compliant.txt" "$compliant" 2>/dev/null; then
        ...
        echo "Using built-in compliant examples"
...
# Main abliterate command – uses the current UPSTREAM model
abliterate() {
    need_preset || return 1
    ...
    # Ensure the original HF model is downloaded
    ...
    echo "Running permanent abliteration on $UPSTREAM"
    echo "Refusal: $refusal ($(wc -l < "$refusal") lines)"
    echo "Compliant: $compliant ($(wc -l < "$compliant") lines)"
    echo "Alpha: $alpha"
    echo "Output: $out"
    ...
    if [ $? -eq 0 ]; then
        echo "Permanent abliteration completed. Model saved to $out"
        echo "Now convert to GGUF and quantize:"
        echo "  python3 /llama.cpp/convert_hf_to_gguf.py $out --outtype bf16 --outfile /gguf/$(basename $UPSTREAM)-abliterated-BF16.gguf"
        echo "Then run your ladder."
    else
...
# One‑click: abliterate → BF16 GGUF → quantize with your existing ladder
abliterate_and_quantize() {
    local alpha="${1:-1.0}"
    local out="${2:-/abliterated-$(basename $UPSTREAM)}"
    ...
    echo "BF16 GGUF ready: $bf16_gguf"
    echo "Now you can quantize it with your ladder:"
